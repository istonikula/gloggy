...
### Tier 14 -- Cursor-Tracking Viewport + Shared Scrolloff (discovered by /ck:check 2026-04-18)

Triggered by user report "I still see no row highlight where cursor is when focused on details pane" + follow-up specifying nvim-style `scrolloff` drag + list-wide coverage. Redefines the detail pane from pure offset-viewport to cursor-tracking viewport; adds shared top-level `scrolloff` config honoured by both entry list and detail pane; integrates search `n`/`N` with cursor semantics. Addresses cavekit-detail-pane R11 (NEW), cavekit-entry-list R12 (NEW), cavekit-config R5 (extended), DESIGN.md §4 matrix + §4.3 + §4.4 + §6 cue #4 + §9 keymap. Execute T-130 first — it is the shared config foundation all other tasks depend on.

| Task | Title | Kit Req | blockedBy | Effort | Description |
|---|---|---|---|---|---|
| T-130 | Top-level `scrolloff` config (shared) | config/R5 | T-003 | S | In `internal/config/config.go` add `Scrolloff int` at the top level of `Config` with TOML tag `scrolloff` and default value 5 in `DefaultConfig()`. In `Load` / parse path, clamp negatives to 0 at load time (per cavekit-config R2 invalid-value handling) and emit a warning. Expose via a getter if a package-private field is preferred. Unit tests: default is 5; a negative value in config is clamped to 0 with warning; an integer value round-trips unchanged; TOML file without `scrolloff` key still loads cleanly with default 5. Do NOT add `entry_list.scrolloff` or `detail_pane.scrolloff` — one key for both panes (cavekit-config R5). Closes F-026 (config foundation). |
| T-131 | Detail-pane cursor state + CursorHighlight render (focused + unfocused-visible) | detail-pane/R11 | T-123, T-078 (CursorHighlight token) | M | In `internal/ui/detailpane/scroll.go` add `cursor int` field to `ScrollModel` (0-indexed document line); initialize to 0 in `NewScrollModel` and `SetContent`. Add `Cursor() int` accessor. In `internal/ui/detailpane/model.go` `View()`, after body composition and before `overlayScrollIndicator` runs, paint the cursor row: compute `visibleCursorRow = m.scroll.cursor - m.scroll.offset`; if `visibleCursorRow ∈ [0, contentH-1]`, wrap that body line in a lipgloss style with `Background(th.CursorHighlight)` spanning the full `contentWidth()`. When `m.Focused` → bg at full intensity + `Bold(true)`; when unfocused-but-visible → bg at reduced intensity via theme.CursorHighlight applied without Bold (per DESIGN.md §4 matrix line 170). Ensure the cursor bg composes correctly with existing syntax-highlight ANSI (use `lipgloss.NewStyle().Inherit` or render row then overlay bg on styled string — prefer composing bg on the raw inner cells so syntax fg stays visible). Unit tests: cursor at line 0 on a 10-line 5-row viewport → visual row 0 has CursorHighlight bg; cursor at line 7 with offset=3 → visual row 4 has bg; unfocused pane → bg present but no Bold; hidden when `!m.open`. Closes F-026 (detail-pane render). |
| T-132 | Detail-pane cursor-tracking navigation + scrolloff follow | detail-pane/R11 | T-130, T-131, T-124 | M | Rewrite `internal/ui/detailpane/scroll.go` `Update()` key handling so it operates on `cursor` first and adjusts `offset` to honour scrolloff: `j`/`down` → `cursor++`, clamped; `k`/`up` → `cursor--`, clamped to ≥ 0; `g`/`home` → cursor=0, offset=0; `G`/`end` → cursor = last, offset = max(0, last - viewport + 1); `pgdown`/`ctrl+d`/`space` → cursor += max(1, viewport-1), offset slides with it; `pgup`/`ctrl+u`/`b` → symmetric. After every navigation apply `followCursor(scrolloff)`: if `cursor < offset + scrolloff` → `offset = max(0, cursor - scrolloff)`; if `cursor > offset + viewport - 1 - scrolloff` → `offset = cursor - viewport + 1 + scrolloff`; then clamp. At document edges scrolloff yields (cursor at line 0 or line len-1 is allowed on the very first/last row). Accept `scrolloff` via a new `WithScrolloff(int)` setter on `ScrollModel` (clamped at use time to `[0, floor(viewport/2)]`). Wire from `app/model.go` at `openPane` + `WindowSizeMsg` by reading `cfg.Scrolloff`. Unit tests: viewport=10, scrolloff=3, cursor starts at 0 → `j` x 6 keeps offset=0 until cursor reaches line 6 (viewport-1-scrolloff = 6); cursor=7 → offset becomes 1; PgDn from top sets cursor+offset correctly; `G` on 100-line doc sets cursor=99, offset=90, visible cursor-row is last; document shorter than `2×scrolloff+1` → clamp proportionally; `scrolloff=0` → cursor always at top/bottom of viewport. Entry-change path (`PaneModel.Open`) resets cursor to 0 and offset to 0. Closes F-026 (detail-pane keyboard nav). |
| T-133 | Detail-pane mouse-wheel scrolloff drag | detail-pane/R11 | T-132 | S | In `internal/ui/detailpane/scroll.go` `Update()` extend `tea.MouseMsg` branch: `MouseButtonWheelDown` increments `offset` by 1 (clamped), then if `cursor < offset + scrolloff` → `cursor = offset + scrolloff` (drag along); `MouseButtonWheelUp` decrements `offset` by 1, then if `cursor > offset + viewport - 1 - scrolloff` → `cursor = offset + viewport - 1 - scrolloff` (drag along). At document edges scrolloff yields. Unit tests: viewport=10, scrolloff=3, 100-line doc, cursor=50, offset=45 (cursor at row 5 from top, well inside middle) → WheelDown x 3 → offset=48, cursor=50 (cursor unchanged, now at row 2 from top = scrolloff-th); WheelDown one more → offset=49, cursor=51 (cursor dragged along to stay at row 2 = scrolloff-th); WheelUp symmetric on upward edge. Closes F-026 (detail-pane mouse wheel). |
| T-134 | Detail-pane search ↔ cursor integration | detail-pane/R7 (extended), R11 | T-131, T-132, T-115 | S | In `internal/ui/detailpane/model.go` `ScrollToLine(idx int)` (T-115), replace "scroll so idx is visible" with "move cursor to idx AND scroll so cursor has scrolloff context". After moving: `m.scroll.cursor = idx; m.scroll = m.scroll.followCursor(scrolloff)`. In `View()`, when rendering the cursor row with `SearchHighlight` on a matched line, compose so `CursorHighlight` bg takes visual priority on the cursor's active row (e.g. render cursor bg on top of search-highlight fg) — other match rows retain their regular SearchHighlight styling. Unit tests: 100-line doc with match on line 40, viewport=10, scrolloff=5 → after `ScrollToLine(40)` cursor=40, offset=35 (cursor at row 5 = middle, 5 rows above + 4 rows below); cursor-row render has CursorHighlight bg (assert ANSI contains the token); search-highlight fg still present on cursor row (token composes). Closes F-026 (detail-pane search). |
| T-135 | Entry-list scrolloff + mouse-wheel drag | entry-list/R12 | T-130 | M | In `internal/ui/entrylist/list.go` locate the existing `scroll.Cursor` + `scroll.Offset` handling (cursor-move + virtual rendering). After every cursor-moving handler (`j`/`k`/`g`/`G`/PgDn/PgUp/Ctrl+d/Ctrl+u + `applyLevelJump` for `e`/`w` + mark-jump for `m`-family), call a new `followCursor(scrolloff int)` helper that mirrors T-132: adjusts `scroll.Offset` so the cursor sits ≥ scrolloff rows away from top/bottom of the currently visible rows count. At list boundaries scrolloff yields (cursor at first/last visible row allowed). Mouse wheel branch: existing WheelDown/WheelUp increments offset; after each, if cursor row would enter the scrolloff margin, drag cursor along (same formula as T-133). When `len(visible) < 2*scrolloff + 1`, clamp `scrolloff` to `floor(len(visible)/2)` at use time. Wire `scrolloff` from `cfg.Scrolloff` via a new `WithScrolloff(int)` setter on ListModel (called from app at `WindowSizeMsg` and on config reload). Unit tests: 200-entry list, viewport=20, scrolloff=5, cursor=0 → `j` x 15 keeps offset=0 until cursor=15 (viewport-1-scrolloff = 14, so at 14 offset starts); cursor=15 → offset=1; PgDn from top correctly adjusts both; level-jump `e` lands cursor on filtered-out ERROR with scrolloff context around it; mark-next-prev honours scrolloff; wheel drag at mid-list keeps cursor on its document entry; wheel near bottom drags cursor along. Closes F-026 (entry-list nav + mouse). |
| T-136 | DESIGN.md §4 matrix + §4.3 + §4.4 + §6 + §9 already updated by /ck:check revise | design-system | — | S | /ck:check 2026-04-18 already amended DESIGN.md (matrix cursor-row scope dropped "(list only)", added §4.3 "Shared scrolloff" subsection, added §4.4 "Cursor and scrolloff" subsection, rewrote §6 focus cue #4, rewrote §9 keymap `j`/`k` row + added Mouse wheel row). This task verifies those edits are internally consistent after T-131..T-135 land (no stale "viewport only" wording left in the doc; all keymap rows match implementation; scrolloff config key appears in §4.3 and §4.4 identically). Append a 2026-04-18 entry to `context/designs/design-changelog.md` after code lands confirming doc-reality alignment. Effort trivial — just a post-impl consistency pass. |
| T-137 | [HUMAN] Cursor-tracking + scrolloff end-to-end sign-off | detail-pane/R11, entry-list/R12 | T-131, T-132, T-133, T-134, T-135, T-136 | M | Per overview "Verification Conventions" §1–§5, verify via tui-mcp on `logs/small.log` (for list) + `logs/tiny.log` line 34 (for detail) across all three bundled themes at `80x24` + `140x35` in both orientations: (1) entry list focused: press `j` repeatedly — cursor visibly moves row-by-row with `CursorHighlight` bg, viewport does not scroll until cursor reaches 5 rows from bottom, then scrolls keeping cursor at that margin; (2) open detail pane (Enter) — list retains focus; Tab to pane — cursor row visible inside detail pane content at line 0; (3) `j`/`k` in focused detail pane visibly moves the cursor row; `G` jumps cursor to end with viewport scrolled so cursor is 5 rows from bottom of content area; (4) `/` + query + `n` lands cursor on match with 5 rows of context above/below where document permits; CursorHighlight bg visible on the active match line above the search highlight; (5) mouse-wheel scroll in the middle of the content: cursor stays on its document line, rows move under it; wheel near an edge drags cursor along to stay in scrolloff margin; (6) focus toggle (Tab): cursor bg in detail pane dims to reduced intensity when unfocused and restores when re-focused; same for list; (7) with config `scrolloff = 0` written to TOML, restart: cursor can reach top/bottom rows freely (no margin). Attach theme + geometry + orientation + `scrolloff` value observations to a new `impl-scrolloff.md` notes file. |

### Tier 15 -- Clipboard Feedback + Detail-Pane Rendering Correctness (discovered by /ck:check 2026-04-18 post-Tier 14)

Triggered by user notes: (list) "copy of marked message does not copy the entries", (details pane) "on small screen lines overflow with max preset width on `tiny.log:1` logger value", (details pane) "strange prop value coloring on `tiny.log:45` when wrapped msg value cursor moves in", (details pane) "cursor bg not shown after prop — `\"prop\": \"value\",` coloring ends before `:` and continues after `,`". Addresses cavekit-app-shell R9 (expanded), cavekit-detail-pane R9/R10/R11 (amended), DESIGN.md §4 matrix contiguity clause. T-138 is independent; T-139/T-140/T-141 can proceed in parallel; T-142 is HUMAN sign-off and must be last.

| Task | Title | Kit Req | blockedBy | Effort | Description |
|---|---|---|---|---|---|
| T-138 | Clipboard feedback + error surfacing + remove `//nolint:errcheck` | app-shell/R9 | T-089 | S | In `internal/ui/app/model.go:408-418` rewrite the `y` handler to consume `ClipboardCopiedMsg` and surface it. Replace `appshell.CopyMarkedEntries(m.entries, markedIDs) //nolint:errcheck` with: compute `count := len(markedIDs)`; if `count == 0` emit `keyhints.WithNotice("no marked entries", 2*time.Second)` and return; otherwise call `msg, err := appshell.CopyMarkedEntries(...)`; on error emit `keyhints.WithNotice("clipboard error: "+err.Error(), 3*time.Second)`; on success emit `keyhints.WithNotice(fmt.Sprintf("copied %d entr%s", msg.Count, plural(msg.Count, "y", "ies")), 2*time.Second)`. Delete the `//nolint:errcheck` comment — it is now a kit violation (R9 AC). `ClipboardCopiedMsg` and `ClipboardErrorMsg` in `internal/ui/appshell/clipboard.go` are already defined — wire them through the notice system (the simplest path: just format directly into `WithNotice` since the Bubble Tea `tea.Msg` roundtrip is unnecessary for a synchronous call). Unit tests: zero marks → notice "no marked entries"; 1 mark → notice "copied 1 entry"; N marks → notice "copied N entries"; simulated `clipboard.WriteAll` error (via test seam or interface) → notice "clipboard error: …". Integration test: `grep -n "nolint:errcheck" internal/ui/app/model.go` returns no matches in the `y` handler. Closes F-102 (P1). |
| T-139 | Fix detail-pane double-subtract border width (single-owner border accounting) | detail-pane/R10 | T-044, T-057 | S | In `internal/ui/detailpane/model.go` `SetWidth(w int)` (~lines 55-74) and `View()` (~lines 459-468), remove the second border subtraction. `internal/ui/appshell/layout.go:66-86` already publishes `DetailContentWidth = usable * ratio` where `usable = TerminalWidth − 2*paneBorders − dividerWidth` — this value is ALREADY post-border. Change `contentWidth() = m.width - 2` to `contentWidth() = m.width` (or delete the helper and use `m.width` directly); same in `View()` where `inner := m.width - 2` currently. Verify no other call site relies on the old double-subtract semantics. Unit tests: `SetWidth(40)` → `contentWidth() == 40`; rendering a line of exactly 40 columns fits within the pane without overflow and without underfill; rendering a line of 41 columns soft-wraps at 40 (not 38). Integration: on `logs/tiny.log` line 1 at `width_ratio = 0.70` on `80x24` right-orientation, logger value `"org.hibernate.validator.internal.util.Version"` renders without overflowing the pane's right edge and without leaving a ≥ 2-col dark seam before it. Closes F-103 (P1). |
| T-140 | Fix soft-wrap to preserve SGR across wrap boundary | detail-pane/R9 | T-043, T-082 | M | In `internal/ui/detailpane/wrap.go:20-35` replace the current `ansi.HardwrapWc(line, width, true)` call with an SGR-preserving wrap. Two acceptable implementations: (a) use `github.com/charmbracelet/x/ansi.Wrap(line, width, "")` which tracks SGR state and re-emits on break; OR (b) keep `HardwrapWc` but post-process each emitted line — at each `\n` boundary, inspect the running SGR state (parse the line's `\x1b[…m` sequences tracking the current fg/bg/attrs) and emit `\x1b[0m` at the end of each wrapped segment + re-emit the active SGR at the start of the next. Preferred: (a). Unit tests: input line `"\x1b[32mkey\x1b[0m: \x1b[36mlongstringvalue\x1b[0m"` wrapped at width 12 → both wrapped segments of `longstringvalue` carry `\x1b[36m` (cyan) at their start; each ends with `\x1b[0m`. Integration: on `logs/tiny.log` line 45 msg value at pane width forcing wrap, all wrapped continuation lines carry the string color (the theme's `String` token), not the key color or no color. Closes F-104 (P1). |
| T-141 | Fix non-contiguous cursor-row background across embedded SGR resets | detail-pane/R11 | T-131 | M | In `internal/ui/detailpane/model.go:342-365` (`paintCursorRow`), the current code applies `lipgloss.NewStyle().Background(th.CursorHighlight).Inline(true).Width(m.contentWidth()).Render(rowString)`; because `rowString` contains embedded `\x1b[0m` resets from per-token `Style.Render()` calls in `internal/ui/detailpane/render.go:42-52, 88-144`, lipgloss's outer SGR does not re-inject across those inner resets and the bg visually terminates at the first reset. Fix: before paint, strip inner `\x1b[0m` sequences on the cursor row (regex `\x1b\[0m` replaced with `""` on the cursor-row string ONLY), then apply the outer bg style. Alternative (cleaner but bigger): introduce `internal/ui/detailpane/render.go` `RenderJSONNoReset(...)` variant that emits tokens without the per-token `\x1b[0m` and relies on the outer caller to close; use it only for the cursor row. Preferred: strip-on-cursor-row (smaller blast radius). Unit tests: `paintCursorRow` called with a string containing `"\x1b[32m\"key\"\x1b[0m: \x1b[36m\"value\"\x1b[0m,"` → output has `Background(CursorHighlight)` SGR opened once, no embedded `\x1b[0m` resets in the cell range `[0, contentWidth)`, and terminates with a single `\x1b[0m` at the end. Integration: on `logs/tiny.log` line 45 with cursor on any JSON property line, `paintCursorRow` output contains no `\x1b[0m` between the first cell and the content-width boundary. Closes F-105 (P1). |
| T-142 | [HUMAN] Clipboard feedback + detail-pane rendering sign-off | app-shell/R9, detail-pane/R9, R10, R11 | T-138, T-139, T-140, T-141 | S | Per overview "Verification Conventions" §1–§5, verify via tui-mcp across all three bundled themes at `80x24` + `140x35` in both orientations: (1) on `logs/small.log`, mark two entries with `m`, press `y` — status bar shows "copied 2 entries" notice that auto-dismisses; mark zero, press `y` — "no marked entries" notice; (2) on `logs/tiny.log` line 1, open detail pane at `width_ratio = 0.70` in right-orientation on `80x24` — logger value fits within pane cells, no overflow past right border, no underfill dark seam; (3) on `logs/tiny.log` line 45, open detail pane, scroll cursor to msg property with wrapped value — wrapped continuation lines of the string value render in the same string color as the first line (no "second half uncolored" / "second half in key color"); (4) on any JSON entry, move cursor row onto a property line like `"key": "value",` — cursor-row `CursorHighlight` bg is visually contiguous across the row, no dark gap at `:`, spaces, `,`, or between styled tokens. Attach theme + geometry + orientation observations to a new `impl-tier15.md` notes file. |

### Tier 16 -- Free-Text List Search (discovered by /ck:check 2026-04-18 post-Tier 14)

Triggered by user note "list: no search". Adds cavekit-entry-list R13 (NEW list search) and re-routes `/` via cavekit-app-shell R13 re-revision (from pane-open-state routing to focus-based routing). T-143 is the list-search implementation; T-144 is the app-shell routing change; T-145 is HUMAN sign-off.

| Task | Title | Kit Req | blockedBy | Effort | Description |
|---|---|---|---|---|---|
| T-143 | Entry-list free-text search model + render | entry-list/R13 | T-114 (search prompt patterns), T-078 (SearchHighlight token), T-135 (scrolloff follow) | L | Add `internal/ui/entrylist/search.go` mirroring `internal/ui/detailpane/search.go` structure: `SearchModel` with `query string`, `matches []int` (indices into the visible filtered entry slice), `current int`, `inputMode bool`, `notFound bool`. API: `Activate()`, `Deactivate()`, `AppendRune(r rune)`, `BackspaceRune()` (UTF-8-safe via `utf8.DecodeLastRuneInString` + slice to rune-length), `Next()`, `Prev()`, `MatchLines() []int`. In `internal/ui/entrylist/list.go` expose `Search() *SearchModel` and thread through `Update` for list-scope `/`, `n`, `N`, Esc keys when `list.Focused() && search.InputMode()`. In `View()` render `SearchHighlight` bg on rows whose index ∈ matches (excluding the cursor row — cursor-row bg takes priority per cavekit-entry-list R13 AC). Also render the query + `(cur/total)` counter in a trailing hint row inside the list (or surface via `keyhints.WithNotice` for consistency with detail-pane search). Implement match scan against the compact-row text (time|level|logger|msg) case-insensitively — reuse the same row-compose helper used by `View()` so matches align with what the user sees. Search state must clear on focus loss (Tab cycle calls `Deactivate()`) and on filter change (`applyFilters` clears `SearchModel`). Unit tests: activate + type "err" → matches all rows with "err" substring; `n` advances cursor to next match; `N` wraps; UTF-8 backspace on emoji query removes exactly one rune; cursor-row bg takes priority over search-highlight on the active match; filter change clears search; focus loss clears search. Closes F-101 (P2) — entry-list R13 implementation. |
| T-144 | Re-route `/` to focus-based dispatch (list vs detail) | app-shell/R13 | T-143, T-121 | S | In `internal/ui/app/model.go` `handleKey` rewrite the `/` branch. Current routing (T-116) focuses detail pane if open or emits "open entry first" notice. New routing (cavekit-app-shell R13 revised): if `m.focus == FocusEntryList` → call `m.list.ActivateSearch()` (new method wrapping `search.Activate()`); if `m.focus == FocusDetailPane` → call `m.pane.ActivateSearch()` (existing); if `m.focus == FocusFilterPanel` → route `/` as a literal character to the filter input (do NOT intercept). Update `internal/ui/appshell/keyhints.go` to publish a focus-sensitive `/` label: `"/ search list"` when list focused, `"/ search pane"` when detail focused, hidden when filter focused. Update the help overlay entry (`internal/ui/appshell/help.go`) from "Search inside detail pane — opens pane if closed" to "Search in focused pane — list or detail". Remove the "open an entry first" transient notice — it no longer applies. Unit tests: list focused + pane closed + `/` → `list.Search().InputMode() == true`; list focused + pane open-but-not-focused + `/` → list search active (NOT pane search); pane focused + `/` → pane search active; filter focused + `/` → filter input receives literal `/`. Closes cavekit-app-shell R13 revision (paired with T-143). |
| T-145 | [HUMAN] List search end-to-end sign-off | entry-list/R13, app-shell/R13 | T-143, T-144 | S | Per overview "Verification Conventions" §1–§5, verify via tui-mcp on `logs/small.log` across all three bundled themes at `80x24` + `140x35` in both orientations: (1) list focused, no detail pane open, press `/` — list search prompt opens (NOT "open entry first" notice); type a substring of a known message — matching rows get `SearchHighlight` bg, `(cur/total)` counter visible; (2) `n` advances cursor onto next match with 5 rows scrolloff context preserved above and below where filtered set permits; `N` wraps at start; Esc clears highlights and keeps cursor position; (3) query yielding zero matches → "No matches" indicator visible; (4) Tab to detail pane (open one first) → list search state clears; Tab back to list → no stale search; (5) open detail pane (Enter then Tab), press `/` → detail-pane search (not list search); (6) open filter panel, press `/` → `/` is typed into the filter input as a literal. Attach theme + geometry + orientation observations to a new `impl-tier16.md` notes file. |

### Tier 17 -- Search Intercept-Ordering + Focus-Loss + Streaming Matches (discovered by /ck:check 2026-04-18 post-Tier 15/16)

Triggered by `/ck:check` inspector findings F-106..F-115. P1 F-106 is a real data-loss regression introduced by T-144-fix (global `q`-quit intercept fires before the search router in input mode). P2 F-108/F-109 are kit-ambiguity gaps codified by the R13 AC 7 broadening + streaming AC additions. P2 F-107 and F-110 + P3 F-111..F-114 are either dead code to remove or deferred quality polish. T-146 is the P1 blocker; T-147/T-148/T-149 close the P2s; T-150 is HUMAN sign-off.

| Task | Title | Kit Req | blockedBy | Effort | Description |
|---|---|---|---|---|---|
| T-146 | Gate `q`-quit intercept behind active-search state (F-106 fix) | app-shell/R14 | T-144 | S | In `internal/ui/app/model.go` `handleKey` (~line 292), the global `q`-to-Quit branch must NOT fire when `m.list.HasActiveSearch() && m.list.Search().InputMode()` (list scope) AND when `m.focus == FocusDetailPane && m.paneSearch.IsActive() && m.paneSearch.Mode() == detailpane.SearchModeInput` (pane scope). Change to: `if msg.String() == "q" && m.focus == appshell.FocusEntryList && !(m.list.HasActiveSearch() && m.list.Search().InputMode()) { ... return m, tea.Quit }`. Regression tests: (1) activate list search → type `q` → `m.list.Search().Query()` ends with `q`, no `tea.Quit` command returned; (2) activate list search → type `query` → query == `"query"`, no Quit; (3) dismiss search via Esc → `q` → Quit returned; (4) pane-search input mode → `q` → typed into pane query, no Quit. Extend `internal/ui/app/model_test.go`. Closes F-106 (P1). |
| T-147 | Deactivate list search on `f` focus transfer (F-108) | entry-list/R13 (AC 7 expanded) | T-144 | XS | In `internal/ui/app/model.go` `handleKey` FocusEntryList branch where `f` transfers focus to the filter panel (~line 427), before changing focus call `if m.list.HasActiveSearch() { m.list = m.list.DeactivateSearch() }`. Mirror of the Tab branch at model.go:316-318. Unit test: list focused + active search + press `f` → `m.focus == FocusFilterPanel && !m.list.HasActiveSearch()`. Adjust any existing `f` tests that assumed search persists. Closes F-108 (P2). |
...
### Tier 20 -- Resize Divider-Coord Alignment + Drag State-Machine Hardening (discovered by /ck:check 2026-04-19 post-Tier 19)

Post-Tier-19 `/ck:check` surfaced that T-156 unit tests validated R15 against the router's own coordinate helpers — so the tests passed trivially while the real user-facing drag initiation on the *visible* divider was broken (F-122 P1). Paired with F-123 (P2, `RatioFromDragY` off-by-one vs `PaneHeight` forward math — Press at current divider snaps ratio down one step), F-125 (P2, drag state survives mid-drag auto-close), F-127 (P3, `rowForY` regression vector if `WithContentTopY` wire is dropped), F-129 (P3, bare Press+Release rewrites config), F-126 (P3, degenerate-terminal drag shadows persisted ratio), F-130 (P3, dead unreachable guard), F-121 (P3 kit-text inversion — already fixed in this /ck:check's R15 AC 4 edit). Tier 20 has 11 tasks (10 auto + 1 HUMAN sign-off).

| Task | Title | Kit Req | blockedBy | Effort | Description |
|---|---|---|---|---|---|
| T-160 | Router/renderer divider-col alignment (F-122) | app-shell/R15 (new renderer-truth AC) | T-094, T-156 | M | Root-cause the 2-col offset between `appshell.MouseRouter.Zone`'s divider column (`listEnd = ListContentWidth()+1`, `divider = listEnd+1`) and Lipgloss's visible divider glyph column (drawn at `ListContentWidth()`). Pick one fix: (a) **rename** `Layout.ListContentWidth()` to `ListSlotWidth()` and audit every call site to confirm outer-vs-inner semantics, then have the router subtract one border, OR (b) **fix the router arithmetic**: `listEnd = ListContentWidth() - 1`, `divider = ListContentWidth()`. Preferred = (b) — smaller blast radius. Then add the renderer-truth test mandated by the new R15 AC: render the layout, find the `│` glyph column programmatically, assert `MouseRouter.Zone(glyphCol, midY) == ZoneDivider` across presets `{0.10, 0.30, 0.50, 0.80}` at 140x35 and 80x24 (and the horizontal divider row equivalent for below-mode). Remove or adjust existing T-156 tests that encode the router's old convention. |
| T-161 | `RatioFromDragY/X` inverse-math fix (F-123) | app-shell/R15 (new inverse-invariant AC) | T-156 | XS | `internal/ui/appshell/ratiokeys.go`: `RatioFromDragY` uses `detail = termHeight - 2 - y` but the correct inverse of `HeightModel.PaneHeight = int(termHeight * ratio)` (given below-mode layout detail occupies rows `entryListEnd+1 .. termHeight-2` with the divider row counting as detail's top border) is `detail = termHeight - 1 - y`. Flip the constant. Add test `TestRatioFromDragY_PressAtCurrentDividerY_KeepsRatio` that computes the current divider Y for each preset `{0.10, 0.30, 0.50, 0.80}` via `HeightModel.PaneHeight`'s inverse, calls `RatioFromDragY` at that Y, and asserts the returned ratio equals the input ratio (no step-snap). Review `RatioFromDragX` for symmetry. |
| T-162 | Mid-drag auto-close termination (F-125) | app-shell/R15 (new mid-drag-auto-close AC) | T-156, T-104 | S | `internal/ui/app/model.go` handleMouse drag branch: at the top of the `if m.draggingDivider` block, check `m.pane.IsOpen()`; if false, set `m.draggingDivider = false` and swallow the motion (return without saving). `WindowSizeMsg` branch: if `ShouldAutoCloseDetail` closes the pane and `m.draggingDivider` is true, clear `draggingDivider` there too (belt-and-braces). New test `TestModel_T162_Drag_Survives_AutoClose_TerminalShrink` — open pane, begin drag via Press at divider, inject `WindowSizeMsg` whose dimensions trigger auto-close, then inject Motion + Release; assert no ratio mutation occurred and config file was not written. |
| T-163 | `rowForY` unset-contentTopY safety (F-127) | entry-list/R10 (new unset-safety AC) | T-158 | XS | `internal/ui/entrylist/list.go`: add a `contentTopYSet bool` field set to true only inside `WithContentTopY`. `rowForY` returns `(-1, false)` when `!contentTopYSet`. New test `TestListModel_T163_RowForY_Rejects_When_ContentTopY_Unset` — construct a fresh `ListModel` without calling `WithContentTopY`, call `rowForY(0)`, `rowForY(5)`, assert all return "no row". Alternative: panic on unset — discuss in commit message; the defensive-return approach is less brittle. |
| T-164 | No-motion drag skips config write (F-129) | app-shell/R15 (new no-motion-no-persist AC) | T-156 | XS | `internal/ui/app/model.go`: add `dragDirty bool` reset alongside `draggingDivider = true` on Press; set to true inside the Motion branch when the ratio actually changes; on Release, call `saveConfig()` only if `dragDirty`. New test `TestModel_T164_BareClick_OnDivider_DoesNotWriteConfig` — Press+Release at divider cell with no Motion in between; `os.Stat` the config file before and after; assert no mtime change. Guard against test flakiness on fast filesystems by using a pre-existing `os.Stat` baseline. |
| T-165 | Degenerate-terminal drag preserves ratio (F-126) | app-shell/R15 (new degenerate-dim AC) | T-156 | XS | `internal/ui/appshell/ratiokeys.go`: both `RatioFromDragX` and `RatioFromDragY` early-return `ClampRatio(RatioDefault)` when `term{Width,Height} <= 0`. Change to return `current` (add a `current float64` parameter and update all call sites in `app/model.go` to pass the pre-drag ratio) — or guard callers to skip `RatioFromDrag*` when the active dimension is ≤ 0. Preferred = caller-guard (smaller API change). New test pinning degenerate-dimension path preserves `current`. |
| T-166 | Remove dead unreachable upper guard (F-130) | app-shell/R15 (no AC; quality) | T-156 | XS | `internal/ui/appshell/ratiokeys.go`: delete the `if detail > termHeight { detail = termHeight }` branch in `RatioFromDragY` (and the symmetric check in `RatioFromDragX` if present) — unreachable because the earlier `< 0` clamp catches any input that could produce `detail > termHeight`. No test; deletion is proven dead by inspection. Add a one-line comment documenting the removal reason to prevent the branch from being re-added. |
| T-167 | DESIGN.md §5 + §9 preset-set sync | DESIGN.md (2 DESIGN VIOLATIONs) | T-155 | XS | `DESIGN.md §5` (ratio keymap table): update preset set from `{0.10, 0.30, 0.70}` to `{0.30, 0.50}`, document focus-aware `+`/`-` direction (acts on focused pane's share), document `=` as focus-independent reset to 0.30. `DESIGN.md §9` (authoritative keymap matrix): same preset update. Add a new row to §9 keymap for R15 mouse-drag on the divider cell (single-persist-on-release, live motion, focus-neutral). Log the change to `context/designs/design-changelog.md`. |
| T-168 | Kit R15 AC 4 physical-layout text fix (F-121) | app-shell/R15 AC 4 | — | XS | Already landed in this `/ck:check`'s kit-amendment pass — AC 4 text flipped from "dragging down grows the detail pane" to "dragging up grows (divider moves up, list shrinks, detail grows)". This task is a placeholder to cross-reference the kit edit; no separate code change required. Mark DONE when the next `/ck:make` cycle starts. |
...
- **Kits changed:** `cavekit-app-shell.md` R15 (+5 new ACs: renderer-truth divider-col assertion, inverse-math invariant, mid-drag auto-close termination, no-motion-no-persist, degenerate-dim guard; AC 4 text physically-corrected from "dragging down grows detail" to "dragging up grows"); `cavekit-entry-list.md` R10 (+1 new AC: list rejects clicks when `contentTopY` unset — defensive lower bound on the single-owner resolver contract).
- **Acceptance-criteria delta:** 291 → 297 (+6).
- **Requirement count delta:** 56 (unchanged — all edits extend existing R-numbers).
- **Tasks added:** 11 (T-160 through T-170). T-160 is the F-122 root-cause fix (router/renderer alignment + renderer-truth test); T-161 is the F-123 one-constant fix with Press-at-current-divider-Y pin; T-162 guards drag state on mid-drag auto-close; T-163 makes `rowForY` unset-safe; T-164 skips no-motion config writes; T-165 preserves current ratio on degenerate-dim; T-166 removes dead upper guard; T-167 syncs DESIGN.md §5 + §9 with the Tier-19 preset set; T-168 is the cross-ref placeholder for the F-121 kit-text fix landed inline; T-169 consolidates impl tracking; T-170 is the HUMAN sign-off gate exercising F-122/F-123/F-125/F-129 through tui-mcp.
- **Tiers added:** 20 (resize divider-coord alignment + drag state-machine hardening). Previous tier ceiling was 19.
- **Root cause recap (F-122):** `Layout.ListContentWidth()` is *ambiguous* between outer (allocated) and inner (content) semantics. `app/model.go:178` sends `WindowSizeMsg{Width: ListContentWidth()}` and the list's handler subtracts 2 for borders — treating it as OUTER. `appshell/mouse.go:66-67` computes `listEnd = ListContentWidth()+1` — treating it as CONTENT. At 140x35 width_ratio=0.3: router thinks divider is at col 96, Lipgloss renders visible `│` at col 94. Unit tests synthesize presses at `ListContentWidth()+2` and agree with the router; no test ever examined a rendered string to locate the actual `│`. **(F-123):** `RatioFromDragY` uses `detail = termHeight-2-y` as the inverse of `PaneHeight = int(termHeight*ratio)`, but the correct inverse given the below-mode layout (detail occupies rows `entryListEnd+1..termHeight-2` with divider row counting as detail's top border) is `termHeight-1-y`. At termHeight=24 and current divider row y=16 (ratio=0.30), the formula returns 0.25 — so a Press snaps ratio down one preset before motion. Tests synthesize motion events from the same helper so the internal inverse passes.
- **Commit history referenced:** T-156 (mouse-drag), T-158 (single-owner click-row resolver), T-094 (mouse zones in right-split), T-104 (right-split width_ratio drag).
