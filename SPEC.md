...
| T11 | ~ | migrate all `*_test.go` to testify: add `github.com/stretchr/testify` to `go.mod`; replace ~1253 `t.Errorf`/`t.Fatalf`/`t.Error`/`t.Fatal` across 58 files with `require.*`/`assert.*`. package-by-package commits for reviewability; `go test ./...` green after each package. | V27 |
