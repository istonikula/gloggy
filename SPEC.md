...
| T25 | . | add unit test in `cmd/gloggy/main_test.go` for ParseArgs piped-stdin cases: swap `os.Stdin` w/ `os.Pipe()` reader (non-tty fd) → assert `FromStdin=true + FollowMode=true` (V23); repeat w/ `-f` flag → same result (`-f` redundant-accepted). restore `os.Stdin` via `defer`. | V23,I.cli |
