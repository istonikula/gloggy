...
| T12 | . | split `internal/ui/app/model_test.go` (~2787 lines) into topic-focused files to restore ergonomics (focus transitions, ratio keys, divider drag, search, resize/lifecycle, misc). pure move refactor — no logic changes, no renames of test funcs, no new coverage. one commit per file-group so blame stays readable. verify `go test ./internal/ui/app/...` green after each split. do AFTER T11 lands so the mechanical testify diff is not entangled with relocations. | V27 |
