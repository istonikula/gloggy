...
func TestTailFile_DetectsNewLines(t *testing.T) {
	...
	for len(tailEntries) < appendCount {
		select {
		case <-timeout:
			require.Failf(t, "timed out", "waiting for tail entries (got %d, want %d)", len(tailEntries), appendCount)
		default:
		}
		...
		case TailStopMsg:
			require.Failf(t, "tail stopped unexpectedly", "%v", m.Err)
		}
	}
	...
	for i, e := range tailEntries {
		...
		assert.Equal(t, want, e.LineNumber, "tailEntries[%d].LineNumber", i)
	}
}
...
func TestTailFile_MultipleAppendBatches(t *testing.T) {
	...
	drainUntil := func(want int, label string) []Entry {
		...
		for len(got) < want {
			select {
			case <-timeout:
				require.Failf(t, label+": timed out", "waiting for tail entries (got %d, want %d)", len(got), want)
			default:
			}
			raw := cmd()
			switch m := raw.(type) {
			case TailStreamMsg:
				...
				cmd = func() interface{} { return nextCmd() }
			case TailStopMsg:
				require.Failf(t, label+": tail stopped unexpectedly", "%v", m.Err)
			}
		}
		return got
	}
	...
	for i, e := range all {
		want := initialLines + i + 1
		assert.Equal(t, want, e.LineNumber, "entry %d LineNumber", i)
	}
}
...
func TestTailFile_EmitsInitialContent(t *testing.T) {
	...
	for len(got) < initialLines {
		select {
		case <-timeout:
			require.Failf(t, "timed out", "waiting for initial emission (got %d, want %d)", len(got), initialLines)
		default:
		}
		raw := cmd()
		switch m := raw.(type) {
...
func TestTailFile_CancelCleansUp(t *testing.T) {
	...
	case <-time.After(3 * time.Second):
		require.Fail(t, "timeout: TailFile goroutine did not clean up after cancel")
	}
}
...
func TestTailFile_InitialDrainEmitsSingleBatch(t *testing.T) {
	...
	case r := <-out:
		...
		require.True(t, ok, "first message: want TailStreamMsg, got %T", r.msg)
		tm, ok := tsm.Unwrap().(TailMsg)
		require.True(t, ok, "first message inner: want TailMsg, got %T", tsm.Unwrap())
		assert.Equal(t, seedLines, len(tm.Entries),
			"initial drain should emit ONE batch of %d entries (per-line emission regresses to N cursor snaps on `gloggy -f` startup)",
			seedLines)
	case <-time.After(5 * time.Second):
		require.Fail(t, "timed out waiting for initial tail drain")
	}
}
...
func TestTailFile_LiveAppendEmitsSingleBatch(t *testing.T) {
	...
	case r := <-out:
		...
		tm, ok := tsm.Unwrap().(TailMsg)
		require.True(t, ok, "first message inner: want TailMsg, got %T", tsm.Unwrap())
		assert.Equal(t, appendK, len(tm.Entries),
			"single Write of %d lines should emit ONE batch of %d entries", appendK, appendK)
	case <-time.After(5 * time.Second):
		require.Fail(t, "timed out waiting for tail append")
	}
}
