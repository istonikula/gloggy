...
func TestNextFocus_ListToDetails(t *testing.T) {
	...
	if got != FocusDetailPane {
		t.Errorf("NextFocus(list, [list,details], false) = %v, want details", got)
	}
}
...
func TestNextFocus_DetailsToList(t *testing.T) {
	...
	if got != FocusEntryList {
		t.Errorf("NextFocus(details, [list,details], false) = %v, want list", got)
	}
}
...
func TestNextFocus_OverlayOpenIsInert(t *testing.T) {
	...
	if got != FocusEntryList {
		t.Errorf("NextFocus with overlayOpen=true must be inert, got %v", got)
	}
	got2 := NextFocus(FocusDetailPane, visible, true)
	if got2 != FocusDetailPane {
		t.Errorf("NextFocus with overlayOpen=true must be inert, got %v", got2)
	}
}
...
func TestNextFocus_SinglePaneIsNoOp(t *testing.T) {
	...
	got := NextFocus(FocusEntryList, visible, false)
	if got != FocusEntryList {
		t.Errorf("NextFocus on single pane = %v, want list", got)
	}
}
...
func TestNextFocus_EmptyVisibleIsNoOp(t *testing.T) {
	...
	if got != FocusEntryList {
		t.Errorf("NextFocus on empty visible = %v, want list (no change)", got)
	}
}
...
func TestNextFocus_CurrentNotInVisible(t *testing.T) {
	...
	got := NextFocus(FocusDetailPane, visible, false)
	if got != FocusEntryList {
		t.Errorf("NextFocus when current not in visible = %v, want list", got)
	}
}
