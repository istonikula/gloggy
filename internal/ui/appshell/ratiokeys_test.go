...
func TestNextDetailRatio_PlusDetailFocus(t *testing.T) {
	...
	require.Truef(t, ok, "+ not recognised")
	assert.InDeltaf(t, 0.35, r, 1e-9, "+ detail 0.30")
}
...
func TestNextDetailRatio_PlusListFocus(t *testing.T) {
	...
	assert.InDeltaf(t, 0.25, r, 1e-9, "+ list 0.30")
}
...
func TestNextDetailRatio_PlusAtMaxDetail_NoOp(t *testing.T) {
	...
	assert.Equalf(t, RatioMax, r, "+ at max detail")
}
...
func TestNextDetailRatio_PlusAtMinList_NoOp(t *testing.T) {
	...
	assert.Equalf(t, RatioMin, r, "+ at min list")
}
...
func TestNextDetailRatio_MinusAtMinDetail_NoOp(t *testing.T) {
	...
	assert.Equalf(t, RatioMin, r, "- at min detail")
}
...
func TestNextDetailRatio_MinusAtMaxList_NoOp(t *testing.T) {
	...
	assert.Equalf(t, RatioMax, r, "- at max list")
}
...
func TestNextDetailRatio_EqualsResetsDefault_BothFocus(t *testing.T) {
	for _, listFocus := range []bool{false, true} {
		...
		assert.Equalf(t, RatioDefault, r, "= listFocused=%v", listFocus)
	}
}
...
func TestNextDetailRatio_PipeDetailFocus_TogglesPresets(t *testing.T) {
	...
	for _, tc := range cases {
		...
		assert.InDeltaf(t, tc.want, got, 1e-9, "| detail from %.2f", tc.from)
	}
}
...
func TestNextDetailRatio_PipeListFocus_TogglesShare(t *testing.T) {
	...
	for _, tc := range cases {
		...
		assert.InDeltaf(t, tc.want, got, 1e-9, "| list from detail=%.2f", tc.from)
	}
}
...
func TestClampRatio(t *testing.T) {
	...
	for _, tc := range cases {
		assert.Equalf(t, tc.want, ClampRatio(tc.in), "ClampRatio(%.2f)", tc.in)
	}
}
...
func TestIsRatioKey(t *testing.T) {
	for _, k := range []string{"+", "-", "=", "|"} {
		assert.Truef(t, IsRatioKey(k), "IsRatioKey(%q): want true", k)
	}
	for _, k := range []string{"j", "k", "esc", "tab", " "} {
		assert.Falsef(t, IsRatioKey(k), "IsRatioKey(%q): want false", k)
	}
}
...
func TestRatioFromDragX_Mid(t *testing.T) {
	...
	want := 45.0 / 95.0
	assert.InDeltaf(t, want, r, 1e-9, "RatioFromDragX(50,100)")
}
...
func TestRatioFromDragX_ClampMax(t *testing.T) {
	...
	assert.InDeltaf(t, RatioMax, r, 1e-9, "RatioFromDragX(2,100)")
}
...
func TestRatioFromDragX_ClampMin(t *testing.T) {
	...
	assert.InDeltaf(t, RatioMin, r, 1e-9, "RatioFromDragX(99,100)")
}
...
func TestRatioFromDragX_NormalizedByWidth(t *testing.T) {
	...
	assert.InDeltaf(t, r1, r2, 0.02, "expected similar ratios at proportional positions")
}
...
func TestRatioFromDragX_PressAtCurrentDividerX_KeepsRatio(t *testing.T) {
	...
	for _, tc := range cases {
		...
		assert.Truef(t, math.Abs(got-tc.ratio) <= RatioStep/2+1e-9,
			"RatioFromDragX(%d, %d) = %.4f; want ≈%.4f (±%.3f) for ratio %.2f",
...
func TestRatioFromDragY_PressAtCurrentDividerY_KeepsRatio(t *testing.T) {
	...
	for _, r := range presets {
		...
		assert.Truef(t, math.Abs(got-r) <= RatioStep/2+1e-9,
			"RatioFromDragY(%d, %d) = %.4f; want ≈%.4f (±%.3f) for ratio %.2f",
